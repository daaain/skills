#!/usr/bin/env node
/**
 * Sync marketplace.json, README.md, and VERSIONS.md with skills directory.
 *
 * Scans the skills/ directory for valid skills (directories containing SKILL.md)
 * and updates marketplace.json, the README skills table, and VERSIONS.md to match.
 */
...
function parseFrontmatter(content) {
  ...
  for (const line of lines) {
    // Skip indented lines (nested YAML like metadata sub-keys)
    if (line.startsWith("  ")) {
      if (currentKey === "metadata" && !frontmatter._metadata) {
      ...
      if (currentKey === "metadata") {
        ...
        if (colonIndex !== -1) {
          ...
          frontmatter._metadata[key] = value;
        }
      }
      continue;
    }
...
function getSkillsWithMetadata() {
  ...
  return fs
    ...
    .map((entry) => {
      ...
      const version = (frontmatter._metadata && frontmatter._metadata.version) || "1.0.0";
...
|-------|---------|--------------|`;
  ...
  const newContent = [header, ...rows, "", recentChanges, ""].join("\n");
...
function main() {
  ...
  const versionsUpdated = updateVersions(skills);

  if (!marketplaceResult.updated && !readmeUpdated && !versionsUpdated) {
